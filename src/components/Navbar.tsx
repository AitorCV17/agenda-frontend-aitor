...
const Navbar: React.FC<{ isSidebarOpen: boolean; toggleSidebar: () => void }> = ({ isSidebarOpen, toggleSidebar }) => {
  ...
  return (
    <nav className="bg-azure-900 dark:bg-azure-950 text-white px-6 py-3 shadow-md flex items-center justify-between transition-all duration-300">
      ...
      {token ? (
        <div className="relative">
          ...
          {menuOpen && (
            <motion.div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 shadow-lg rounded-lg py-2" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
              ...
              <Link to="/edit-profile" className="block px-4 py-2 text-gray-700 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                Editar Perfil
              </Link>
              <button onClick={handleLogout} className="w-full text-left px-4 py-2 text-red-500 hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                Cerrar Sesión
              </button>
            </motion.div>
          )}
        ...
        <div>
          <Link to="/login" className="mr-4 text-azure-300 hover:text-azure-200 transition">Iniciar Sesión</Link>
          <Link to="/register" className="text-azure-300 hover:text-azure-200 transition">Registrarse</Link>
        </div>
      )}
    </nav>
  );
