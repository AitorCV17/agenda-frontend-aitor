...
const ShareTaskListModal: React.FC<ShareTaskListModalProps> = ({ listId, onClose }) => {
  ...
  const handleShare = async () => {
    try {
      ...
      onClose()
      alert('Lista de tareas compartida con éxito')
    } catch (err: any) {
      setError(err.response?.data?.message || 'Error al compartir la lista')
    }
  }
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white p-6 rounded shadow-md w-80">
        <h2 className="text-xl font-bold mb-4">Compartir Lista de Tareas</h2>
