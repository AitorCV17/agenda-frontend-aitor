...
const ShareEventModal: React.FC<ShareEventModalProps> = ({ eventId, onClose }) => {
  ...
  // Cargar usuarios compartidos al abrir el modal
  useEffect(() => {
    fetchSharedUsers()
  }, [eventId])
  ...
  const handleShare = async () => {
    try {
      ...
      fetchSharedUsers() // Refresca la lista después de compartir
    } catch (err: any) {
  ...
  const handleRevoke = async (userId: number) => {
    try {
      ...
      fetchSharedUsers() // Refresca la lista después de revocar
    } catch (err: any) {
