...
const ShareTaskListModal: React.FC<ShareTaskListModalProps> = ({ listId, onClose }) => {
  ...
  const fetchSharedUsers = async () => {
    try {
      ...
      const res = await axios.get(`/tasks/lists/${listId}/shares`);
  ...
  const handleShare = async () => {
    try {
      ...
      setError('');
      setSuccessMessage('');
      await axios.post(`/tasks/lists/${listId}/share`, {
      ...
      setError(err.response?.data?.message || 'Error al compartir la lista de tareas');
    } finally {
  ...
  const handleRevoke = async (userId: number) => {
    try {
      ...
      setError('');
      setSuccessMessage('');
      await axios.delete(`/tasks/lists/${listId}/share/${userId}`);
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-2xl w-full max-w-md p-6 relative transition-all" ref={modalRef}>
        <button
          onClick={onClose}
          ...
          {loadingShares ? (
            ...
            <ul className="space-y-2">
              {sharedUsers.map(u => (
                ...
                >
                  <div>
                    <p className="font-medium text-azure-700 dark:text-azure-300">{u.email}</p>
